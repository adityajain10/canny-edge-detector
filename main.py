    ...
    for row in range(image_height):
        for col in range(image_width):
            ...
            if horizontal_gradient[row][col] == 0:
                ...
                gradient_angle[row][col] = math.degrees(math.atan((vertical_gradient[row][col]) / (horizontal_gradient[row][col])))
...
def double_thresholding(suppressed_image, gradient_angle):
    ...
    for i in index_weak_edge_pixel:
        ...
        if 0 < x < image_width and 0 < y < image_height:
            ...
            if (((suppressed_image[x - 1][y] in strong_edge_pixel) and abs(gradient_angle[x - 1][y] - pixel_gradient) > 45) or
                    ((suppressed_image[x + 1][y] in strong_edge_pixel) and abs(gradient_angle[x + 1][y] - pixel_gradient) > 45) or
                    ((suppressed_image[x][y + 1] in strong_edge_pixel) and abs(gradient_angle[x][y + 1] - pixel_gradient) > 45) or
                    ((suppressed_image[x][y - 1] in strong_edge_pixel) and abs(gradient_angle[x][y - 1] - pixel_gradient) > 45) or
                    ((suppressed_image[x - 1][y + 1] in strong_edge_pixel) and abs(gradient_angle[x - 1][y + 1] - pixel_gradient) > 45) or
                    ((suppressed_image[x - 1][y - 1] in strong_edge_pixel) and abs(gradient_angle[x - 1][y - 1] - pixel_gradient) > 45) or
                    ((suppressed_image[x + 1][y - 1] in strong_edge_pixel) and abs(gradient_angle[x + 1][y - 1] - pixel_gradient) > 45) or
                    ((suppressed_image[x + 1][y - 1] in strong_edge_pixel) and abs(gradient_angle[x + 1][y - 1] - pixel_gradient) > 45)):
